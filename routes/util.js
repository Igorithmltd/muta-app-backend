 ...
 *                     imageUrl:
 *                       type: string
 *                       example: "https://yourdomain.com/uploads/document123.jpg"
 *                     publicId:
 *                       type: string
 *                       example: "document123"
 *       400:
...
router.post(
  ...
  auth,
  (req, res, next) => {
    document_uploader.single("file")(req, res, (err) => {
      if (err) {
        return res.status(400).json({
          success: false,
          error: err.message,
  ...
  (req, res) => {
    const utilController = new UtilController();
    return utilController.uploadDocument(req, res);
  }
);
